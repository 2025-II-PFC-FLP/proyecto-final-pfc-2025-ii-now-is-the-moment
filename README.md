[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/h71fa0_C)
# Asignación: [PROYECTO FINAL]

**Fecha:** [9/12/2025]
**Curso:** [Fundamentos de Programacion Funcional y Concurrente]

---

## 👥 Integrantes del Grupo

| Nombre Completo             | Código    | Rol           | Correo Electrónico                          |
|-----------------------------|-----------|---------------|---------------------------------------------|
| Juan Esteban Perez Ramirez  | 202459486 | [Líderisimo]  | [juan.perez.ramirez@correounivalle.edu.co]  |
| Bryan Steven Ospina         | 202459353 | [Colaborador] | [bryan.steven.ospina@correounivalle.edu.co] |
| Lopez Botero Karen          | 202459519 | [Colaborador] | [karen.sofia.lopez@correounivalle.edu.co]   |
| Marlen Lizeth Estrada López | 202359494 | [Colaborador] | [marlen.estrada@correounivalle.edu.co]      |
| [Estudiante 5]              | 123458    | [Colaborador] | [correo3@institucion.edu]                   |
---

## 📌 Descripción de la Asignación

Este taller busca analizar el impacto de la paralelización en la optimización de la programación de riego de una finca, evaluando el desempeño de versiones secuenciales y paralelas del algoritmo. Los objetivos incluyen medir y comparar tiempos de ejecución, identificar los efectos del overhead de paralelización y comprender en qué condiciones el cómputo paralelo resulta beneficioso. Para ello se aplican conceptos vistos en clase como programación funcional, estructuras paralelas, recursión y análisis de rendimiento.
